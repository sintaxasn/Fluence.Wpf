...
- **Full-suite WPF UI test ordering** - Some template-state UI tests can fail only when the entire multi-target suite is run in one process, while passing in isolation. Observed examples include the `net472` ComboBox focus-line pair, the `net472` ScrollBar mouse-indicator width test, and the `net10.0` tab-selection content test. Use isolated reruns to confirm regressions until the test harness fully resets WPF focus, template, and dispatcher state between tests.
