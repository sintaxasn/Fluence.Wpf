...
- **Fluent typography propagation** - text-bearing controls now explicitly use the shared `FluentFontFamily` token and propagate it through content presenters, placeholders, headers, menu text, password reveal text, and list/tree/tab item content while retaining ClearType/fixed-hint text options.
