...
- `NavigationView`'s selection indicator travels between items instead of blinking out and back in. It used to fade to nothing over 90 ms, jump the gap, and fade back in over 140 ms, so it never visibly crossed the space between the two items. It now holds full opacity and plays WinUI's stretch-and-settle: one continuous move over 600 ms with the bar elongating to span the gap by the 33 percent mark and contracting onto the destination, on WinUI's own bezier pair. WinUI reaches the same picture with two per-item indicators animating together, which a single pane-level bar cannot do, so the stretch is carried by one scale instead.
