...
- **FluenceWindow title-bar metrics** - default `FluenceWindow` minimum width is now 698 px and default `TitleBarHeight` remains 68 px; the demo shell uses a compact 42 px title bar, the `NavigationView` header reservation matches that 42 px chrome, and title text now returns whenever it has search-box clearance.
...
- **NavigationView and ProgressRing parity** - aligned `NavigationView` chrome with the WinUI ordering (`Back`, then pane toggle), added `NavigationView.IsPaneToggleButtonVisible`, collapsed disabled back buttons instead of showing an inactive slot, moved demo title-bar navigation chrome into the extended title bar while keeping search visible, and replaced the indeterminate `ProgressRing` motion with the 5 second linear keyframe arc from the Avalonia reference.
...
- **Window page accent swatches refreshed** - `GalleryColorsPage` retains the full theme brush catalogue; the Window page accent picker now presents seven rainbow swatches selected so generated Fluence accent fills keep at least 4.5:1 contrast with `TextOnAccentFillColorPrimary`.
