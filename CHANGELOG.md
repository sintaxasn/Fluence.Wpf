...
- **NavigationView title-bar alignment** - extended-title-bar demo chrome now uses 48 px rail slots so back/collapse glyphs align with left-mode item icons, title identity shifts right as glyphs appear, and non-extended left pane chrome uses a horizontal row above the item list without the previous extra spacer.
- **FluenceWindow title-bar metrics** - default `FluenceWindow` minimum width is now 698 px and default `TitleBarHeight` is 68 px; the demo shell uses the same values and keeps extended-title-bar NavigationView items below the title-bar chrome.
