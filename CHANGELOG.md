...
- **NavigationView Top/Left parity** - `NavigationView` now synchronizes its nearest `FluenceWindow` title-bar extension for `Left` and `Top` pane modes, coerces top panes open with the pane toggle hidden, keeps top item icons and text visible, removes top-strip scrolling, and moves overflowed top items into a three-dot popup without reparenting the real `NavigationViewItem` containers.
- **Gallery sample parity** - inputs now include horizontal and vertical tick-mark slider examples, the repeat button sample shows a live click count without the extra accent repeat button, selection samples cover two-state and three-state CheckBox usage plus a Select All tri-state group, RatingControl examples, the requested two ToggleSwitch rows with a bound ProgressRing, visible ListView / TreeView sample borders, and a TreeView multi-select checkbox sample.
