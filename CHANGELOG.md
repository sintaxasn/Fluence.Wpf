...
- `ComboBox`: the dropdown now opens with the canonical translate-plus-fade reveal (8 px slide toward the control edge with an opacity fade over 167 ms on the standard spline, matching the FlyoutPresenter precedent) instead of unfolding from a vertical scale of zero; upward-opening dropdowns slide up accordingly.
- `Flyout`: the open reveal now slides 8 px in from the side the flyout is placed on (the default `Top` placement slides up, `Bottom` slides down, `Left`/`Right` slide horizontally) instead of always sliding down regardless of placement; the 167 ms duration, fade, and `0.8,0,0,1` spline are unchanged. The reveal keys off the requested placement side; a screen-edge flip to the opposite edge still plays the requested side's reveal.
- `CheckBox`: becoming checked now animates the check glyph in (100 ms fade plus a 167 ms scale settle from 0.7 on the `0.8,0,0,1` spline), approximating WinUI's AnimatedIcon draw-in; unchecking remains instant.
- Demo motion polish (demo only): gallery page navigation now uses just the 167 ms fade masking the page swap (the decorative 20 px slide is removed), and the color-swatch hover scale is softened from 1.08 over 120 ms to 1.05 over the 100 ms `ControlPressAnimationDuration` value.
- `ProgressBar`: determinate value changes now animate the fill's `ScaleTransform.ScaleX` instead of its layout `Width`, so each frame of the 367 ms reposition animation is composite-only (no layout or paint per frame); the fill is laid out once at the full track width and relies on the indicator host's rounded geometry clip for corner rounding. The 367 ms duration and (0.1,0.9,0.2,1.0) spline are unchanged; the template gains a `PART_FillScale` part.
