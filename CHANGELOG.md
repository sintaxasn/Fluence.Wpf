...
- **`TreeViewSelectionMode` and TreeView multi-selection** - `TreeView.SelectionMode` now supports `None`, `Single`, and `Multiple`, `TreeView.SelectedItems` exposes the live selected item list, and nullable `TreeViewItem.IsSelectionChecked` drives checked, unchecked, and indeterminate multiple-selection checkbox states.
...
- **Demo shell pane motion and top overflow** - the demo search box now caps its responsive title-bar width at 475 px, the title-bar Back glyph renders in a 16 x 16 px visual box, the Top toggle keeps the switch left of its label and left-aligns in Left pane mode, top overflow fills all available item space before the three-dot entry reaches the Top toggle status, and pane-mode transitions sequence item-list, top-toggle, and content motion through the requested Left, LeftCompact, and Top layouts.
- **Demo shell navigation polish** - the title-bar Back slot now uses a compact 36 x 32 px target while the pane toggle keeps its 42 x 36 px slot, the gallery Back action walks real visited-page history instead of always returning Home, long title text yields before the centered search box, and the top overflow button reserves a 12 px end clearance so it does not overlap the last clipped item.
