...
- **Gallery sample parity** - inputs now include horizontal and vertical tick-mark slider examples, the repeat button sample shows a live click count, selection samples cover two-state and three-state CheckBox usage plus a Select All tri-state group, RatingControl examples, ToggleSwitch plus ProgressRing, and a TreeView multi-select checkbox sample.
...
- **`TreeViewSelectionMode` and TreeView multi-selection** - `TreeView.SelectionMode` now supports `None`, `Single`, and `Multiple`, `TreeView.SelectedItems` exposes the live selected item list, and `TreeViewItem.IsSelectionChecked` drives the multiple-selection checkbox state.
