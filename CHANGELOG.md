...
- A flyout shown from a button's `Click` stays open. A light-dismiss popup takes the mouse capture when it opens, and a button still holds that capture while its `Click` handler runs, so the flyout took the capture mid-gesture and lost it again the moment the button let go, closing as it appeared. The popup now opens pinned and takes the capture a dispatcher turn later, once the gesture has been processed and nothing is about to hand the capture back. `ShowAt` stays synchronous, and a click away still dismisses the flyout.
