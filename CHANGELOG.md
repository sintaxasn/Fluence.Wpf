...
- **ProgressRing rewrite to WinUI canonical 5-dot orbit (replaces caterpillar arc)** — the previous `StrokeDashArray`-driven sweep approximation (and its later `IndeterminateOffsetProperty` caterpillar fix) is gone.  The new template hosts five `Ellipse` orbit dots inside individually-rotated wrapper `Grid`s (`E1R`…`E5R` rotate transforms; dots `E1`…`E5`).  A `ControlTemplate.Triggers` `MultiTrigger` on `(IsActive=True, IsIndeterminate=True)` starts the canonical 3.47 s spline-keyframe storyboard sourced verbatim from `microsoft-ui-xaml-main/src/controls/dev/ProgressRing/TestUI/ProgressRingStoryboardAnimationPage.xaml` — five staggered dots (begin times 0, 0.167, 0.334, 0.501, 0.668 s) trace a 7-keyframe spline path from −110° to 585° each with the same `KeySpline` set used by WinUI.  All animation is XAML-driven; no code-behind animation state remains.  Code-behind shrinks to (1) compute `EllipseDiameter` / `EllipseOffset` template settings on size change (matching `ProgressRingTemplateSettings.cpp` math: `diameter = (width × 0.1) + (width ≤ 40 ? 1 : 0)`, `anchor = (width × 0.5) − diameter`) and (2) render the determinate arc geometry via `ArcSegment` with a 150 ms ease-in-out tween on `Value` changes.  The `IndeterminateSweep` / `IndeterminateOffset` private DPs and `CreateIndeterminateSweepAnimation` internal helper are removed.
...
- **GalleryStatusPage ProgressRing demo condensed** — the duplicate "Indeterminate" and "Active (spinning)" columns (both `IsActive=True, IsIndeterminate=True`, visually identical after the 5-dot orbit rewrite) are collapsed into a single `ProgressRing` (`x:Name="IndeterminateProgressRing"`) with a `ToggleSwitch` below it bound two-way to `IsActive` via `ElementName`. The `UniformGrid` column count drops from 4 to 3. The determinate arc and inactive columns are retained unchanged.
