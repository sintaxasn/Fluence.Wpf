...
- Demo: all 17 gallery catalog pages now share a `GalleryPageHeader` (title plus docs link, theme toggle, and favorite actions) instead of a bare title, mirroring the WinUI Gallery item page header. The docs button opens the matching `docs/controls.md` section on GitHub and hides itself on pages with no matching section (Home, Colors); the theme button flips Light/Dark while preserving the shell's current backdrop, and is inert (and disabled) while High Contrast is active, since that is a distinct accessibility theme rather than a point on the Light/Dark axis; the favorite star toggle keeps in-page state only, with no persistence.
...
- CheckBox now defaults to `HorizontalAlignment=Left` and `MinWidth=120` (WinUI parity); focus visuals no longer stretch across full-width containers.
...
- Accessibility: `PasswordBox` now forwards its accessible name and `AutomationProperties.LabeledBy` to the inner focusable password and reveal fields during `OnApplyTemplate`. Those inner elements receive keyboard focus, so a screen reader previously announced only a bare protected edit field when focus landed inside; the caller's prompt (set via `AutomationProperties.LabeledBy` or a name on the control) is now the field's spoken label.
- NavigationView content layer is now an opaque pre-blend (#F9F9F9 light / #272727 dark) so the light theme content area no longer renders darker than the pane over Mica.
- NavigationView content layer now follows the desktop wallpaper tint like WinUI's Mica-backed layer (computed opaque pre-blend, re-applied on wallpaper change); neutral wallpapers keep #F9F9F9/#272727.
- FluenceWindow no longer paints an accent window border; the frame always uses the subtle system stroke, matching WinUI 3 Gallery.
