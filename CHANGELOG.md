...
- `ListView.ViewState` does what its name says: `GridView` swaps the items panel for a wrapping one, the way WinUI's own `GridView` uses an `ItemsWrapGrid`, and `Default` puts the vertical panel back. The property was registered with no callback and read nowhere, so setting it did nothing at all. A panel a consumer has pinned through `ItemsPanel` still outranks it, and the grid layout gives up virtualization, which WPF has no wrapping panel to keep.
- `ProgressBar.ShowStepMarkers` does what its name says: a step progress bar is notched at each interior step boundary, through the track and the fill alike, so it reads as a row of segments. The property was settable and read nowhere.
- A flyout shown from a button's `Click` stays open. A light-dismiss popup takes the mouse capture when it opens, and a button still holds that capture while its `Click` handler runs, so the flyout took the capture mid-gesture and lost it again the moment the button let go, closing as it appeared. `FlyoutBase.ShowAt` now takes the capture a second time once the gesture is over. The method stays synchronous: the popup is open by the time it returns, as before.
...
- `ComboBoxItem`'s corner radius is the keyed `ComboBoxItemCornerRadius` (3, which WinUI deliberately keeps distinct from `ControlCornerRadius`) rather than a literal in the template, so an application overriding corner radii can reach it.
