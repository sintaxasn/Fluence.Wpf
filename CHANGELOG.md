...
- **ProgressRing arc backwards motion (caterpillar fix)** — replaced `RepeatBehavior.Forever` sweep animation with a single-cycle `Completed`-chained architecture (`StartNextIndeterminateCycle`). Added `IndeterminateOffsetProperty` (private DP) that drives `StrokeDashOffset` at exactly the same rate sweep shrinks during contraction, so the leading arc edge advances at a constant `1/1.4 rot/s` with no backwards motion. `_cumulativeOffset` accumulates 0.65 per cycle; each new cycle receives a fresh `from` value so no visual reset jump occurs at the cycle boundary.
