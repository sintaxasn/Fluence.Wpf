...
- Template parts: for new code, `const string PART_Whatever = "PART_Whatever"`, so the identifier and its value are the same text; annotate the class with `[TemplatePart(Name = PART_..., Type = typeof(T))]`. Five in-tree constants predate this convention and pair a `PartWhatever` identifier with a value that is not the same text: `NavigationView` `PartPaneColumn = "PaneColumn"`, `ToggleSwitch` `PartSwitchKnob = "SwitchKnob"` and `PartSwitchThumb = "SwitchThumb"` (all three still carry `[TemplatePart]`), and `PasswordBoxExtensions` `PartMainBorder` and `PartPlaceholder`, resolved through `Template.FindName` rather than `GetTemplateChild` since `PasswordBox` is sealed and the class carries no `[TemplatePart]` attribute at all. Do not extend that older form to a new template part. A private constant naming a plain `x:Name` that is not a template part contract, for example `DatePicker` `SegmentsHostName` or `TreeViewItem` `SelectionCheckBoxPart`, keeps ordinary PascalCase and no attribute. Part constants are `private`, with one exception: the nine on `NavigationView` are `internal` because the NavigationView tests read them and `InternalsVisibleTo("Fluence.Wpf.Tests")` already exists. Do not add a second exception.
