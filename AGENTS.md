...
- `ApplicationThemeManager.Apply(ApplicationTheme theme, WindowBackdropType backdrop = WindowBackdropType.Auto, bool updateAccent = true)` - first call seeds all three slots; later calls replace `[0]` with a freshly built computed dictionary. `updateAccent` is accepted for signature compatibility but no longer branches behavior.
