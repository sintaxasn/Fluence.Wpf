...
| Packaging | Planned | SourceLink, portable PDBs, a `.snupkg` symbol package, and `ContinuousIntegrationBuild` on CI are not yet in the tree; `DebugType` is `full` everywhere today. The tag-gated GitHub release job (zipped per-TFM binaries, the demo, and the nupkg) already exists, but the NuGet push step in `.github/workflows/build.yml` stays commented out as a deliberate manual release step. |
...
| Item | Value | Cost |
| --- | --- | --- |
| SourceLink and symbol package | Consumers can step into library sources from a debugger. | Two properties, a package reference, and a `.snupkg` push step to maintain. |
| PublicApiAnalyzers post-1.0 freeze | Makes an accidental breaking change a build error rather than a review miss. | Two tracking files per project that every intentional API change must update. |
