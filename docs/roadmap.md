...
| `InfoBar` close pipeline | `Closed` is raised inline at one site instead of through the shared raise path, and a `Closing` handler that sets `IsOpen` to `false` yields two `Closed` events. | The `Closed` signature and `InfoBarCloseReason` are frozen at 1.0, so the re-entrancy guard is a behavioural fix for a minor release; the double `Closed` is recorded in `KNOWN_ISSUES.md` until then. |
...
| Reproducible test baselines | `Fluence.Wpf.Tests/Baselines/*.txt` embed a machine-absolute path and a `duration:` line, so a contributor who regenerates them on another machine gets a spurious diff. | Strip both lines in the capture step documented in `Fluence.Wpf.Tests/README.md` and regenerate once. |
